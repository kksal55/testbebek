...
//Otomatik mesaj için hazırlanan kısım. haziruyarimesaji isimli field de eklenecek 
$m_hatalibaslik='Hesabınızın askıya alınmaması için;
Soru başlıklarını belirlerken lütfen aşağıdaki kurallara uyunuz. Soru başlığını belirlerken sorunuzu kısaca özetleyen bir cümle yazmaya çalışınız. Kurallara uymayan sorularınızı acilen düzenleyiniz.

AKSİ TAKTİRDE HESABINIZ OTOMATİK OLARAK  SİSTEM TARAFINDAN ASKIYA ALINABİLİR.
...
gibi cümleler KULLANMAYINIZ. Sorunuzu sormadan önce benzer sorular olup olmadığını lütfen kontrol ediniz.

Soru başlığında ve soru detayında imla kurallarına dikkat edelim. Dikkat edelim ki diğer annelerimizde bu platformdan en iyi şekilde faydalansın. Burası senin,benim hepimizin platformu......
--------';
$m_siddet = "Değerli kullanıcımız,
Haklı veya haksız platformda kaba söz içeren veya tahrik edici içerik paylaşmak yasaktır. Aksi taktirde yaptırımla karşılaşmak zorunda kalırsınız. Eğer kurallara aykırı mesaj yazılmışsa uyarı butonuna basarak bizlere bildirmeniz yeterli. Zaten gün içerisinde uyarı alan cevapların kontrolünu sağlıyoruz.";
$m_multihesap="Değerli kullanıcımız,
Bir kullanıcının sadece bir hesabı olabilir. Birden fazla hesap açılması hesapların kısıtlanmasına neden olmaktadır.";
$m_tekrarkonu="Değerli kullanıcımız,
Aynı sorunun tekrar tekrar paylaşılması yasaktır. Sorunuzu paylşatıktan sonra cevaplanması için beklemeniz grekmektedir.";
$gorunsunmu="";
if (qa_get_logged_in_level() >= QA_USER_LEVEL_EDITOR) {
$haziruyarimesaji =  array(
            'id' => 'otomesajf',
			'label' => '<i style="font-size:11px;color:gray;">Otomatik Uyarı Mesajı:</i>',
			'type' => 'select',
			...
			'options' => array(
                                '' => "Seçiniz",
                                $m_hatalibaslik => "Hatalı başlık seçimi",
                                $m_siddet => "Şiddet içerikli paylaşım",
