			...
			if(!empty($blobin)) 
			{
				...
				if(isset($blobdata))
				{
					...
					if($blobdata['format']=='jpg' || $blobdata['format']=='jpeg')
					{
						...
						// is the image an avatar
						$isavatar = qa_db_read_one_assoc( qa_db_query_sub('
							SELECT userid FROM ^users 
							WHERE avatarblobid = #
							', $blobid), 
						true);

						// if the image is an avatar, we need to change the qa_users table and swap the values of avatarwidth and avatarheight
						if(!empty($isavatar))
						{
							qa_db_query_sub('
							UPDATE ^users as t, 
							(
								SELECT avatarwidth as aw,  avatarheight as ah 
								FROM ^users 
								WHERE avatarblobid = # 
							) as temp
							SET avatarwidth = temp.ah, avatarheight = temp.aw
							WHERE t.avatarblobid = #
							', $blobid, $blobid);

							// also clear old cached image
							qa_db_query_sub('
								DELETE FROM ^cache 
								WHERE cacheid = #
								',
